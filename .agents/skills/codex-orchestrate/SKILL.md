...
Durable ledgers are not automatic runtime logs. When a trigger applies, final review is blocked until the root creates the ledger or explicitly states why it could not. Inside this repo, use `python3 scripts/create_orchestration_ledger.py` for repo-local ledgers or `--global-output` for `~/.codex/orchestration-ledgers/`, `python3 scripts/orchestration_check.py --quick` for tiered validation, and `python3 scripts/serve_orchestration_ui.py` for read-only dashboard review; elsewhere write the same JSON shape to `~/.codex/orchestration-ledgers/` or use `docs/codex-orchestrate/run-ledger-template.md` manually. Keep private ledgers local/global or sanitized before committing.
