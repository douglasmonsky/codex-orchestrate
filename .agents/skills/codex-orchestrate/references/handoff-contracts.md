...
Durable ledgers are not automatic runtime logs. When a trigger applies, final review is blocked until the root creates the ledger or explicitly states why it could not. In the `codex-orchestrate` repository, use `python3 scripts/create_orchestration_ledger.py` for repo-local ledgers or add `--global-output` to write under `~/.codex/orchestration-ledgers/` or `$CODEX_HOME/orchestration-ledgers/`. In other repos, write the same JSON shape to the Codex-home orchestration ledger directory or use `docs/codex-orchestrate/run-ledger-template.md` manually. Keep private ledgers local/global or sanitized before committing.
